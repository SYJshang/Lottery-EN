...
#import "SYJHotDeailCell.h"

#define numWidth (KSceenW - 140) / 5

@implementation SYJHotDeailCell

+ (instancetype)cellWithTableView:(UITableView *)tableView{
    
    static NSString *cellId = @"detailHotCell";
    SYJHotDeailCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (!cell) {
        cell = [[SYJHotDeailCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
    }
    return cell;
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.firstLab.sd_layout.leftSpaceToView(self.contentView, 70).topSpaceToView(self.timeLab, 10).heightIs(numWidth).widthIs(numWidth);
