...
#define numWidth (KSceenW - 150) / 5


@implementation SYJHotLottoCell


+ (instancetype)cellWithTableView:(UITableView *)tableView{
    
    static NSString *cellId = @"hotCell";
    SYJHotLottoCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (!cell) {
        cell = [[SYJHotLottoCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
    }
    return cell;
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.firstLab.sd_layout.leftSpaceToView(self.contentView, 75).topSpaceToView(self.timeLab, 10).heightIs(numWidth).widthIs(numWidth);
        ...
        line.sd_layout.leftSpaceToView(self.contentView, 60).rightSpaceToView(self.contentView, 60).topSpaceToView(self.powerPlayLab, 5).heightIs(0.5);
