...
#define numWidth (KSceenW - 200) / 3

@implementation SYJPickCell

+ (instancetype)cellWithTableView:(UITableView *)tableView{
    
    static NSString *cellId = @"pickCell";
    SYJPickCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (!cell) {
        cell = [[SYJPickCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
    }
    return cell;
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.firstLab.sd_layout.leftSpaceToView(self.contentView, 100).topSpaceToView(self.timeLab, 10).heightIs(numWidth).widthIs(numWidth);
...
//        line.sd_layout.leftSpaceToView(self.contentView, 60).rightSpaceToView(self.contentView, 60).topSpaceToView(self.thirdLab, 5).heightIs(0.5);
        ...
        self.NextTimeLab.sd_layout.heightIs(20).topSpaceToView(self.thirdLab, 8).leftSpaceToView(self.contentView, 20).rightSpaceToView(self.contentView, 20);
